...
module.exports = {
  ...
  createSale: async (req) => {
    try {
      ...
      if (payment_method.toLowerCase() === "split" && !split) {
      ...
      if (
        ...
        (Object.keys(split).length !== 2 ||
          !Object.entries(split).every(([key, value]) => value))
      ) {
        return sendServiceMessage(
      ...
      if (
        ...
        split["cash"] + split["bank"] !== total_amount
      ) {
        return sendServiceMessage(
      ...
      for (const item of items) {
        ...
        const recalculatedSubTotal = +Math.round(
          item_total / (1 + gst_rate / 100)
        );
        const recalculatedGST = +Math.round(
          recalculatedSubTotal * (gst_rate / 100)
        );
        const recalculatedRate = +Math.round(recalculatedSubTotal / quantity);
      ...
      if (payment_status === "Due") {
        ...
        const bankAccount = await BankAccount.findByPk(company.bank_account_id);
        if (!bankAccount) {
          return sendServiceMessage(
            "messages.apis.app.sale.create.invalid_bank_account"
          );
      ...
      //     .toFixed(1);
      //   const tax = items
      //     .reduce((sum, item) => sum + item.amount * (gstRate / 100), 0)
      //     .toFixed(1);
      ...
      //     savingsPercentage: totalDiscount
      //       ? ((totalDiscount / (totalAmount + totalDiscount)) * 100).toFixed(2)
      //       : "0",
    ...
    try {
      ...
      const cashTransactions = await FinancialTransaction.findAll({
        where: {
          branch_id,
          employee_id: biller_id,
          transaction_date: { [Op.between]: [dayStart, dayEnd] },
  ...
  createHandoverRequest: async (req) => {
    try {
      const { manager_id, cash_amount, timestamp, register_id } = req.body;
